...
class AIClient:
    ...
    async def chat_with_data(self, prompt: str, group_id: str = None,
        ...
        async with AI_SEMAPHORE:
            return await self._chat_with_data_impl(prompt, group_id, user_id, system_prompt)

    async def _chat_with_data_impl(self, prompt: str, group_id: str = None,
                                    user_id: int = None,
                                    system_prompt: str = None) -> str:
        """原有的 chat_with_data 逻辑"""
        ...
        # 获取当前用户的 admin_id（用于数据隔离）
        admin_id = get_user_admin_id(user_id)

        # ✅ 新增：严格校验 admin_id
        if user_id != OWNER_ID and (admin_id is None or admin_id <= 0):
            return "�� 无法确定您的管理员归属，请联系超级管理员处理。\n\n错误代码: INVALID_ADMIN_ID"

        # 权限检查：需要完整权限（管理员或正式操作员）
        ...
        # ✅ 新增：在所有数据查询前再次校验 admin_id
        if admin_id is None or admin_id <= 0:
            return "❌ 数据查询需要有效的管理员身份，当前身份无效。\n\n错误代码: DATA_ACCESS_DENIED"

        # 获取数据（传入 admin_id）
        data = await self._fetch_data(intent, data_provider, user_id, admin_id)
        if data.get("error"):
        ...
        for config in self.configs:
            ...
            try:
                ...
                if "insufficient" in error_msg.lower() or "quota" in error_msg.lower() or "balance" in error_msg.lower():
                            self.failed_keys.add(config["name"])  # 全局标记（配额耗尽影响所有人）
                else:
                    local_failed.add(config["name"])  # 本地标记（网络问题只影响当前请求）
                continue
    ...
    async def _identify_intent(self, prompt: str, user_id: int = 0, admin_id: int = 0) -> Dict:
        """
        识别用户意图 - 使用规则匹配
        """
        prompt_lower = prompt.lower()
        ...
        if has_address_keyword or address_note:
            ...
            my_addresses = get_monitored_addresses(admin_id=admin_id, user_id=user_id)
...
💡 **提示**：模糊匹配 | 5分钟上下文记忆 | 先查询后导出"""
    ...
    async def _fetch_data(self, intent: Dict, data_provider, user_id: int = 0, admin_id: int = 0) -> Dict:
        """根据意图获取数据"""
        ...
        if intent_type == "operators":
            result = data_provider.get_operators(admin_id=admin_id)
            # 直接返回，不要做额外处理
            return result

        # 群组分类
        if intent_type == "group_categories":
            return data_provider.get_group_categories(admin_id=admin_id)

        # 新加入群组
        if intent_type == "joined_groups_today":
            return data_provider.get_today_joined_groups(admin_id=admin_id)
        if intent_type == "joined_groups_yesterday":
            return data_provider.get_yesterday_joined_groups(admin_id=admin_id)
        if intent_type == "joined_groups_weekly":
            return data_provider.get_weekly_joined_groups(admin_id=admin_id)
        if intent_type == "joined_groups_monthly":
            return data_provider.get_monthly_joined_groups(admin_id=admin_id)
        ...
        if intent_type == "activity_ranking":
            return data_provider.get_group_activity_ranking(admin_id=admin_id)
        if intent_type == "active_groups":
            date = params.get("date", "today")
            if date == "yesterday":
                return data_provider.get_yesterday_active_groups(admin_id=admin_id)
            elif date == "week":
                return data_provider.get_week_active_groups(admin_id=admin_id)
            elif date == "month":
                return data_provider.get_month_active_groups(admin_id=admin_id)
            else:
                return data_provider.get_today_active_groups(admin_id=admin_id)
        if intent_type == "top_group":
            return data_provider.get_today_top_group(admin_id=admin_id)

        # 用户相关
        if intent_type == "top_users":
            return data_provider.get_today_top_users(params.get("limit", 10))
        if intent_type == "active_users":
            return data_provider.get_today_active_users(admin_id=admin_id)

        # 数据分析
        if intent_type == "hourly_distribution":
            return data_provider.get_hourly_distribution(admin_id=admin_id)
        if intent_type == "category_percentage":
            return data_provider.get_category_income_percentage(admin_id=admin_id)
        if intent_type == "weekly_trend":
            return data_provider.get_weekly_trend(admin_id=admin_id)
        if intent_type == "month_total":
            return data_provider.get_month_total_income(admin_id=admin_id)
        ...
        if intent_type == "pending_groups":
            return data_provider.get_pending_usdt_groups(admin_id=admin_id)
        if intent_type == "today_summary":
            return data_provider.get_today_summary(admin_id=admin_id)

        # 所有群组收入
        if intent_type == "today_all_income":
            date = params.get("date", "today")
            if date == "yesterday":
                result = data_provider.get_yesterday_all_income(admin_id=admin_id)
            elif date == "week":
                result = data_provider.get_week_all_income(admin_id=admin_id)
            elif date == "month":
                result = data_provider.get_month_all_income(admin_id=admin_id)
            else:
                result = data_provider.get_today_all_income(admin_id=admin_id)
        ...
        # 待下发
        if intent_type == "pending_groups":
            result = data_provider.get_pending_usdt_groups(admin_id=admin_id)
            # 🔥 如果返回结果中有 summary，直接使用
            if result.get("summary"):
                return result
            return result
        ...
        if intent_type == "group_bill":
            ...
            if date == "today":
                return data_provider.get_group_today_bill(admin_id=admin_id, group_name=group_name)
            elif date == "yesterday":
                return data_provider.get_group_yesterday_bill(admin_id=admin_id, group_name=group_name)
            elif date == "week":
                return data_provider.get_group_week_bill(admin_id=admin_id, group_name=group_name)
            elif date == "month":
                return data_provider.get_group_month_bill(admin_id=admin_id, group_name=group_name)
            elif "_to_" in date:
                parts = date.split("_to_")
                return data_provider.get_group_bill_range(admin_id=admin_id, group_name=group_name, start_date=parts[0], end_date=parts[1])
            return data_provider.get_group_bill_by_date(admin_id=admin_id, group_name=group_name, date_str=date)

        # 对比分析
        if intent_type == "group_compare":
            return data_provider.get_group_compare(admin_id=admin_id, group_name=params.get("group_name", ""), period=params.get("period", "today_vs_yesterday"))
        if intent_type == "all_compare":
            return data_provider.get_all_compare(admin_id=admin_id, period=params.get("period", "today_vs_yesterday"))

        # 分类群组
        if intent_type == "groups_by_category":
            return data_provider.get_groups_by_category(admin_id=admin_id, category_name=params.get("category", ""))
        if intent_type == "all_groups_by_category":
            return data_provider.get_all_groups_by_category(admin_id=admin_id)
        ...
        if intent_type == "address_stats":
            ...
            if not address and not note:
                ...
                return {"error": "请指定要查询的地址", "suggestion": f"您已添加的地址：{', '.join(notes)}\n\n💡 例如：查询 {notes[0]} 今日收入"}
            if note:
                for addr in my_addresses:
            ...
            if not address:
                # 检查是否是别人添加的地址
                all_addresses = get_monitored_addresses(admin_id=None, user_id=None)  # 获取所有地址？不，仅用于检查
                # 简化：直接返回错误
                return {"error": f"未找到备注为「{note}」的监控地址", "suggestion": f"您已添加的地址：{[a.get('note', a['address'][:8]) for a in my_addresses]}"}
            try:
                return await data_provider.get_address_stats(address, date, user_id=user_id, admin_id=admin_id)
            except Exception as e:
                print(f"地址统计查询失败: {e}")
        ...
        if intent_type == "address_monthly_stats":
            ...
            if not address:
                return {"error": f"未找到备注为「{note}」的监控地址", "suggestion": f"您已添加的地址：{[a.get('note', a['address'][:8]) for a in my_addresses]}"}
            try:
                return await data_provider.get_address_monthly_stats(address, user_id=user_id, admin_id=admin_id)
            except Exception as e:
                print(f"地址月度统计查询失败: {e}")
                return {"error": f"查询失败: {str(e)[:100]}"}

        return {"error": "无法识别的查询类型"}
