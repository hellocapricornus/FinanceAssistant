...
# --- 核心逻辑：同步与清理（保持不变，但需要传入 admin_id）---
async def sync_and_clean_groups(context: ContextTypes.DEFAULT_TYPE, admin_id: int):
    """同步并清理无效群组（针对单个管理员）"""
    db_groups = get_all_groups_from_db(admin_id=admin_id)
    valid_groups = []
    ...
    for g in db_groups:
        ...
        try:
            ...
            if chat.title != g['title']:
                ...
                save_group(admin_id, gid, chat.title, g.get('category', '未分类'))
            valid_groups.append(g)
        except Exception as e:
            logger.warning(f"检测到已退出群组，正在删除：{gid} ({g['title']}) - {e}")
            delete_group_from_db(admin_id, gid)
...
async def start_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("in_broadcast"):
        await query.answer("已经在群发设置中", show_alert=True)
        return
    # 获取管理员ID
    admin_id = get_user_admin_id(user_id)
    if admin_id == 0:
        await query.answer("❌ 您不是任何管理员，无法使用群发功能", show_alert=True)
        return
    # 完整权限检查（禁止临时操作人）
    ...
    if not groups:
        # ✅ 修改：用 answer + edit_text 替代 reply_text，避免重复提示
        await query.answer("⚠️ 未找到任何有效群组", show_alert=True)
        try:
            await query.message.edit_text("⚠️ **未找到任何有效群组**\n\n请确保机器人已添加到群组中。")
        except Exception:
    ...
    context.user_data["bc_admin_id"] = admin_id
...
async def show_group_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """渲染群组选择界面（支持分类筛选，基于管理员独立库）"""
    if update.callback_query:
        user_id = update.callback_query.from_user.id
    else:
        user_id = update.effective_user.id
    admin_id = context.user_data.get("bc_admin_id", get_user_admin_id(user_id))
    groups = context.user_data.get("bc_all_groups", [])
    # 重新过滤（确保隔离）
    groups = [g for g in groups if g.get('admin_id', 0) == admin_id]
    context.user_data["bc_all_groups"] = groups
    ...
    text += f"👇 点击群名勾选/取消："
    keyboard = []
    # 分类筛选按钮（仅显示当前管理员独立库中实际使用的分类）
    used_categories = set()
    for g in groups:
        cat = g.get('category', '未分类')
        used_categories.add(cat)
    all_categories = get_all_categories(admin_id=admin_id)
    filtered_cats = [cat for cat in all_categories if cat['name'] in used_categories or cat['name'] == '未分类']
    if filtered_cats:
    ...
    for g in display_groups:
        ...
        keyboard.append([InlineKeyboardButton(f"{icon} [{category}] {safe_title}", callback_data=f"bc_toggle_{gid}")])
    # 分页导航
    nav_buttons = []
    if current_page > 0:
    ...
    cancel_row = [InlineKeyboardButton("❌ 取消群发并返回主菜单", callback_data="bc_cancel_and_exit")]
...
async def bc_back_to_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keys = ["bc_all_groups", "bc_selected_ids", "bc_message_content", "bc_temp_target_ids",
            "bc_selected_category", "bc_batches", "bc_current_batch", "bc_batch_results",
            "bc_waiting_for_next", "in_broadcast", "bc_current_page", "bc_admin_id"]
    for k in keys:
        context.user_data.pop(k, None)
    context.user_data.pop("active_module", None)
    from handlers.menu import get_main_menu
    await query.message.edit_text("请选择功能：", reply_markup=get_main_menu(user_id), parse_mode=None)
...
async def bc_cancel_and_exit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keys = ["bc_all_groups", "bc_selected_ids", "bc_message_content", "bc_temp_target_ids",
            "bc_selected_category", "bc_batches", "bc_current_batch", "bc_batch_results",
            "bc_waiting_for_next", "in_broadcast", "active_module", "bc_current_page", "bc_admin_id"]
    for k in keys:
        context.user_data.pop(k, None)
    from handlers.menu import get_main_menu
    try:
        await query.message.edit_text("✅ 已取消群发\n\n请选择功能：", reply_markup=get_main_menu(user_id))
    except Exception:
        await query.message.reply_text("✅ 已取消群发\n\n请选择功能：", reply_markup=get_main_menu(user_id))
    return ConversationHandler.END
...
async def bc_prepare_send(update: Update, context: ContextTypes.DEFAULT_TYPE, mode: str):
    ...
    try:
        await query.message.reply_text(
            f"📝 确认发送配置\n\n{mode_text}\n🎯 目标数量：{len(target_ids)} 个\n\n"
            f"👉 请发送要群发的内容：\n\n支持的类型：\n• 📝 文字消息\n• 🖼️ 图片（可附带说明）\n• 🎬 视频（可附带说明）\n• 📎 文件\n• 🎭 GIF动图\n\n"
            f"💡 提示：输入 /cancel_broadcast 取消",
            parse_mode=None, reply_markup=cancel_markup
        )
    except Exception as e:
        logger.error(f"发送提示消息失败: {e}")
        await query.message.reply_text(
            f"📝 <b>确认发送配置</b>\n\n{mode_text}\n🎯 目标数量：{len(target_ids)} 个\n\n"
            f"👉 请输入要发送的消息内容：\n(支持 Markdown，输入 /cancel_broadcast 取消)\n\n"
...
async def bc_batch_send_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"📦 将分 {len(batches)} 批发送\n每批最多 {batch_size} 个群，批次间隔5秒\n\n总群组数：{len(all_groups)} 个\n\n开始发送第一批？",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶️ 开始发送", callback_data="bc_start_batch")]])
    )
    return BC_SELECT_GROUPS
...
async def bc_execute_batch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, group in enumerate(batch):
        ...
        try:
            await context.bot.send_message(chat_id=gid, text=message_text, parse_mode="Markdown")
            success += 1
        ...
        if (i + 1) % 20 == 0 or i == len(batch) - 1:
            try:
                await progress_msg.edit_text(
                    f"📦 **批次 {current_batch + 1}/{len(batches)} 发送中...**\n本批次进度：{i+1}/{len(batch)}\n"
                    f"✅ 成功：{success} | ❌ 失败：{failed}\n\n📊 **总计**\n✅ 成功：{batch_results['success']} | ❌ 失败：{batch_results['failed']}"
                )
            except:
                pass
    if current_batch + 1 < len(batches):
        await progress_msg.edit_text(
            f"✅ **批次 {current_batch + 1} 完成！**\n本批次：成功 {success}，失败 {failed}\n\n等待5秒后开始下一批...\n或点击「下一批」立即开始",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏩ 下一批", callback_data="bc_next_batch")]])
        )
        ...
        await progress_msg.edit_text(
            f"🎉 **所有批次发送完成！**\n\n📊 **最终统计**\n✅ 成功：{batch_results['success']}\n❌ 失败：{batch_results['failed']}\n"
...
async def receive_message_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prefs = get_user_preferences(user_id)  # 需要传入 admin_id？但这里我们需要从独立库读取，所以需要先获取 admin_id
    admin_id = context.user_data.get("bc_admin_id", get_user_admin_id(user_id))
    # 注意：get_user_preferences 在 db.py 中已修改为需传入 admin_id
    ...
    if message.text and message.text.strip() == "/cancel_broadcast":
        keys = ["bc_all_groups", "bc_selected_ids", "bc_message_content", "bc_temp_target_ids",
                "bc_selected_category", "bc_batches", "bc_current_batch", "bc_batch_results",
                "bc_waiting_for_next", "bc_current_page", "bc_admin_id"]
        for k in keys:
            context.user_data.pop(k, None)
        context.user_data.pop("active_module", None)
        from handlers.menu import get_main_menu
    ...
    if message_data["type"] == "text":
        ...
        caption = f"📋 **发送预览**\n\n类型：图片\n" + (f"说明：{message_data['caption'][:100]}\n" if message_data.get('caption') else "") + f"\n🎯 目标数量：{count} 个群\n\n✅ 确认发送吗？"
        await update.message.reply_photo(photo=message_data["file_id"], caption=caption, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    elif message_data["type"] == "video":
        caption = f"📋 **发送预览**\n\n类型：视频\n" + (f"说明：{message_data['caption'][:100]}\n" if message_data.get('caption') else "") + f"\n🎯 目标数量：{count} 个群\n\n✅ 确认发送吗？"
        await update.message.reply_video(video=message_data["file_id"], caption=caption, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    elif message_data["type"] == "document":
        caption = f"📋 **发送预览**\n\n类型：文件\n文件名：{message_data.get('filename', '未知')}\n" + (f"说明：{message_data['caption'][:100]}\n" if message_data.get('caption') else "") + f"\n🎯 目标数量：{count} 个群\n\n✅ 确认发送吗？"
        await update.message.reply_document(document=message_data["file_id"], caption=caption, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    elif message_data["type"] == "animation":
        caption = f"📋 **发送预览**\n\n类型：GIF动图\n" + (f"说明：{message_data['caption'][:100]}\n" if message_data.get('caption') else "") + f"\n🎯 目标数量：{count} 个群\n\n✅ 确认发送吗？"
        await update.message.reply_animation(animation=message_data["file_id"], caption=caption, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    elif message_data["type"] == "audio":
        caption = f"📋 **发送预览**\n\n类型：音频\n" + (f"说明：{message_data['caption'][:100]}\n" if message_data.get('caption') else "") + f"\n🎯 目标数量：{count} 个群\n\n✅ 确认发送吗？"
        await update.message.reply_audio(audio=message_data["file_id"], caption=caption, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    elif message_data["type"] == "sticker":
        await update.message.reply_sticker(sticker=message_data["file_id"], reply_markup=InlineKeyboardMarkup(keyboard))
        await update.message.reply_text(
            f"📋 **发送预览**\n\n类型：贴纸\n\n🎯 目标数量：{count} 个群\n\n✅ 确认发送吗？",
            reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown"
        )
...
async def execute_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def send_one(gid):
        try:
            ...
            if msg_type == "text":
                await context.bot.send_message(chat_id=gid, text=message_data["content"], parse_mode="Markdown")
            elif msg_type == "photo":
                await context.bot.send_photo(chat_id=gid, photo=message_data["file_id"], caption=message_data.get("caption", ""))
            elif msg_type == "video":
                await context.bot.send_video(chat_id=gid, video=message_data["file_id"], caption=message_data.get("caption", ""))
            elif msg_type == "document":
                await context.bot.send_document(chat_id=gid, document=message_data["file_id"], caption=message_data.get("caption", ""))
            elif msg_type == "animation":
                await context.bot.send_animation(chat_id=gid, animation=message_data["file_id"], caption=message_data.get("caption", ""))
            elif msg_type == "audio":
                await context.bot.send_audio(chat_id=gid, audio=message_data["file_id"], caption=message_data.get("caption", ""))
            elif msg_type == "sticker":
                await context.bot.send_sticker(chat_id=gid, sticker=message_data["file_id"])
            return True
        except Exception as e:
            logger.error(f"发送失败 {gid}: {e}")
            return False
    for batch_start in range(0, total, batch_size):
        ...
        failed += batch_failed
        await progress_msg.edit_text(
            f"🚀 **群发进行中...**\n批次 {batch_num}/{total_batches}\n进度：{batch_start + len(batch)} / {total}\n"
            f"✅ 成功：{success} | ❌ 失败：{failed}\n⏱️ 等待 {delay_between} 秒..."
        )
...
async def end_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keys = ["bc_all_groups", "bc_selected_ids", "bc_message_content", "bc_temp_target_ids",
            "bc_selected_category", "bc_batches", "bc_current_batch", "bc_batch_results",
            "bc_waiting_for_next", "bc_current_state", "in_broadcast", "bc_current_page", "bc_admin_id"]
    for k in keys:
    ...
    try:
        if update.callback_query and update.callback_query.message:
            await update.callback_query.message.edit_text("请选择功能：", reply_markup=get_main_menu(user_id))
        elif update.message:
            await update.message.reply_text("请选择功能：", reply_markup=get_main_menu(user_id))
        elif update.effective_chat:
            await update.effective_chat.send_message("请选择功能：", reply_markup=get_main_menu(user_id))
    except Exception as e:
...
async def bc_fallback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("bc_temp_target_ids") and not context.user_data.get("bc_message_data"):
        if update.message:
            await update.message.reply_text("📝 请发送要群发的内容（文字、图片、视频等）\n或输入 /cancel_broadcast 取消")
        return ConversationHandler.END
    keys = ["bc_all_groups", "bc_selected_ids", "bc_message_content", "bc_temp_target_ids",
            "bc_selected_category", "bc_batches", "bc_current_batch", "bc_batch_results",
            "bc_waiting_for_next", "bc_current_page", "bc_message_data", "bc_admin_id"]
    for k in keys:
        context.user_data.pop(k, None)
    from handlers.menu import get_main_menu
    try:
...
async def bc_cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update.callback_query:
            await update.callback_query.message.edit_text("❌ 已取消群发\n\n请选择功能：", reply_markup=get_main_menu(user_id))
        elif update.message:
            await update.message.reply_text("❌ 已取消群发\n\n请选择功能：", reply_markup=get_main_menu(user_id))
    except Exception as e:
...
async def bc_cancel_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"取消操作失败: {e}")
        if update.effective_chat:
            await update.effective_chat.send_message("❌ 已取消群发\n\n请选择功能：", reply_markup=get_main_menu(user_id))
    return ConversationHandler.END

def get_handlers():
    return [
        ConversationHandler(
            entry_points=[CallbackQueryHandler(start_broadcast, pattern="^(func_broadcast|broadcast|bc_start_real)$")],
            states={
                BC_SELECT_GROUPS: [
                    ...
                    CallbackQueryHandler(bc_page_prev, pattern="^bc_page_prev$"),
                    CallbackQueryHandler(bc_page_next, pattern="^bc_page_next$"),
                ...
                BC_INPUT_MESSAGE: [
                    MessageHandler(~filters.COMMAND, receive_message_input),
                    CallbackQueryHandler(bc_cancel_and_exit, pattern="^bc_cancel_and_exit$"),
                ],
                BC_CONFIRM_SEND: [
...
async def start_broadcast_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_id = get_user_admin_id(user_id)
    if admin_id == 0:
        await update.message.reply_text("❌ 您不是任何管理员，无法使用群发功能")
        return
    context.user_data["in_broadcast"] = True
    keys = ["bc_all_groups", "bc_selected_ids", "bc_message_content", "bc_temp_target_ids",
            "bc_selected_category", "bc_batches", "bc_current_batch", "bc_batch_results",
